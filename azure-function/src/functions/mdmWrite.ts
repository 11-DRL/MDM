...
async function overrideField(req: HttpRequest, ctx: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    await execSql(`
      ...
      VALUES (CONVERT(BINARY(32), @hk, 2), 'override_field', @fieldName, @newValue, @caller, GETUTCDATE(), @reason)
    `, { hk: body.locationHk, fieldName: body.fieldName, newValue: body.newValue, caller, reason: body.reason });
